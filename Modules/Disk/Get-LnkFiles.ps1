...
try {
    if (Test-path ($LECmdParserOutputPath)) {
        $suppress = Remove-Item $LECmdParserOutputPath -Force -Recurse
    } 
}
catch {
    Write-Error "Failed to remove Output folder: $LECmdParserOutputPath."
}